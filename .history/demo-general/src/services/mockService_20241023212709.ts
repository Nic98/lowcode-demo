...
export const async getProjectSchemaFromLocalStorage = (scenarioName: string) => {
  if (!scenarioName) {
    console.error('scenarioName is required!');
    return;
  }
  // const localValue = window.localStorage.getItem(getLSName(scenarioName));
  const localValue = getProjectSchemaFromDB(scenarioName);
  if (localValue) {
    return JSON.parse(await localValue);
  }
  return undefined;
}
